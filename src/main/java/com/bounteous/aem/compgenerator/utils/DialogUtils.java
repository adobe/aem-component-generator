		...
		try {
			...
			if (property.getItems() != null && property.getItems().size() > 0) {
				if (!property.getType().equalsIgnoreCase("multifield")) {
					...
					// Simple multifield with one field
					if (property.getItems().size() == 1) {
					...
					} else { // Multifield with multiple fields
						...
						for (Property item : property.getItems()) {
							...
							if (StringUtils.isNotEmpty(item.getLabel())) {
								optionNode.setAttribute(Constants.PROPERTY_FIELDLABEL, item.getLabel());
							}
							if (StringUtils.isNotEmpty(item.getDescription())) {
								optionNode.setAttribute(Constants.PROPERTY_FIELDDESC, item.getDescription());
							}
